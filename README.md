...
Детальний приклад запиту до агента:

```
Проаналізуй існуючий проєкт як brownfield-систему та зафіксуй baseline-специфікації поточного стану без вигадування нових вимог.

Працюй тільки в межах change `baseline-current-system`.

## Мета
1. Створити або оновити `openspec/changes/baseline-current-system/specs/*/spec.md` так, щоб вони відображали лише фактичну поточну поведінку системи.
2. За потреби створити або оновити `openspec/changes/baseline-current-system/design.md` як as-is design поточної системи.
3. Після цього підготувати baseline до злиття в головні спеки.

## Джерела істини
Використовуй лише те, що можна підтвердити з:
- продакшн-коду;
- автотестів;
- конфігурації, схем, міграцій;
- API-контрактів;
- документації в репозиторії;
- інших артефактів, що прямо підтверджують поточну поведінку або поточну архітектуру.

Якщо щось неможливо підтвердити — не включай це як факт. Познач як gap / open question.

## Правила
- Не імплементуй нові фічі.
- Не змінюй продакшн-код, тести, конфіги або runtime-файли.
- Не описуй бажаний або майбутній стан системи.
- Не роби припущень без підтвердження.
- Не змішуй фактичні вимоги з ідеями покращень.
- Працюй лише з baseline change-артефактами та головними спеками.

## Вимоги до spec.md
Для кожного capability зафіксуй:
- призначення;
- межі відповідальності;
- акторів / зовнішні системи, якщо підтверджено;
- функціональні вимоги;
- обмеження, валідації;
- помилки / edge cases, якщо вони явно реалізовані;
- залежності / інтеграції, якщо вони підтверджені;
- gaps / open questions.

Формулюй вимоги чітко, перевірно і без імплементації. Фіксуй лише те, що система реально робить зараз.

## Вимоги до design.md
Якщо система має нетривіальну архітектуру, кілька підсистем, інтеграції або складні потоки даних — створи або онови `openspec/changes/baseline-current-system/design.md`.

У `design.md` зафіксуй лише as-is design:
- основні компоненти / модулі;
- межі відповідальності між ними;
- ключові потоки даних і взаємодії;
- зовнішні інтеграції;
- сховища, сервіси, черги та інші архітектурно значущі елементи, якщо вони підтверджені;
- технічні обмеження та компроміси, якщо вони явно випливають із системи;
- gaps / неоднозначності / суперечності.

Не включай:
- target architecture;
- refactoring proposal;
- ідеї покращень;
- непідтверджені висновки.

## Merge в головні спеки
Після заповнення baseline-артефактів:
1. порівняй baseline-спеки з головними спеками;
2. перенеси лише підтверджені актуальні вимоги;
3. прибери дублювання;
4. не перенось gaps як факти;
5. якщо в головних спеках є застарілі або непідтверджені вимоги — явно познач їх як такі, що потребують перегляду.

## Формат фінального звіту
Покажи:
1. список capability;
2. що створено, а що оновлено;
3. короткий зміст кожного capability;
4. чи був створений / оновлений `design.md` і що в ньому зафіксовано;
5. що було змерджено в головні спеки;
6. gaps / open questions;
7. ризики baseline-аналізу.
```
